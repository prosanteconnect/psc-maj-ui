  ...
  group "psc-maj-ui-services" {
    ...
    task "psc-maj-ui" {
      ...
      config {
        ...
        volumes = [
          "name=psc-maj-ui-data,io_priority=high,size=3,repl=3:/app/files-repo"
        ]
        volume_driver = "pxd"
        ports = ["http"]
      }
      template {
        data = <<EOH
{{ with secret "psc-ecosystem/psc-maj-ui" }}{{ .Data.data.certificate }}{{ end }}
EOH
        destination = "secrets/certificate.pem"
      }
      template {
        data = <<EOH
{{ with secret "psc-ecosystem/psc-maj-ui" }}{{ .Data.data.private_key }}{{ end }}
EOH
        destination = "secrets/key.pem"
      }
      template {
        data = <<EOH
{{ with secret "psc-ecosystem/psc-maj-ui" }}{{ .Data.data.cacerts }}{{ end }}
EOH
      ...
      template {
        ...
        env = true
        data = <<EOH
PUBLIC_HOSTNAME={{ with secret "psc-ecosystem/psc-maj-ui" }}{{ .Data.data.public_hostname }}{{ end }}
...
EOH
      ...
      template {
        data = <<EOF
server.servlet.context-path=/psc-maj-ui/v1
...
extract.download.url={{ with secret "psc-ecosystem/psc-maj-ui" }}{{ .Data.data.extract_download_url }}{{ end }}
test.download.url={{ with secret "psc-ecosystem/psc-maj-ui" }}{{ .Data.data.test_download_url }}{{ end }}
use.ssl=true
enable.scheduler={{ with secret "psc-ecosystem/psc-maj-ui" }}{{ .Data.data.enable_scheduler }}{{ end }}
...
deactivation.excluded.profession.codes={{ with secret "psc-ecosystem/psc-maj-ui" }}{{ .Data.data.deactivation_codes_exclusion_list }}{{ end }}
...
psc-maj-ui.mail.receiver={{ with secret "psc-ecosystem/emailing" }}{{ .Data.data.mail_receiver }}{{ end }}
EOF
      ...
      service {
	    name = "$\u007BNOMAD_JOB_NAME\u007D"
        tags = ["urlprefix-$\u007BPUBLIC_HOSTNAME\u007D/psc-maj-ui/v1/"]
        port = "http"
        check {
          type = "http"
          path = "/psc-maj-ui/v1/check"
          port = "http"
          interval = "30s"
          timeout = "2s"
